const fetchData = async () => {
    try {
        ...
        if (result.data && Array.isArray(result.data) && result.data.length > 1) {
            ...
            // Function to extract month name and year from date
            const getMonthNames = (dateStr) => {
                console.log("Date String Received:", dateStr); // Debug log
                ...
                const bulanTransaksi = `${months[month - 1]} ${year}`; // Current month and year
                const bulanMasukTagihan = `${months[month % 12]} ${month === 12 ? year + 1 : year}`; // Next month and year

                // Log extracted bulan values
                console.log("Extracted Bulan Transaksi:", bulanTransaksi);
                console.log("Extracted Bulan Masuk Tagihan:", bulanMasukTagihan);
            ...
            transactionMonthElements.forEach(element => {
                element.textContent = bulanTransaksi; // Use bulanTransaksi
            });
